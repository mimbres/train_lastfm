...
parser.add_argument("-s","--save_path",type=str, default="./save/exp_lastfm_top50_regressor_sigmoid_L1BCE_4layer_7/")
...
class MLP_Regressor(nn.Module):
    def __init__(self, input_d=INPUT_DIM, output_d=OUTPUT_DIM,
                 emb_d=EMBEDDING_DIM, mid_d=512): # emb_dim means the l-1 layer's dimension
...
scheduler = StepLR(optim, step_size=200, gamma=0.6)  
...
def Train():
    ...
    for epoch in trange(0, EPOCHS, desc='epochs', position=0, ascii=True):
        ...
        for it in range(len(tr_iter)):
            ...
            total_trloss_L1 += l1.item()
            total_trloss_BCE += l2.item()
        ...
        tqdm.write("tr_epoch:{0:}  tr_loss_L1:{1:.9f}  tr_lossBCE:{2:.9f}  val_loss:{2:.9f}".format(epoch, 
                   total_trloss_L1/total_items, total_trloss_BCE/total_items, hist_vloss[-1]))
...
def Validate():
    ...
    samples = (x.cpu().detach().numpy()[:5,:], y.cpu().detach().numpy()[:5,:],
               y_hat_sig.cpu().detach().numpy()[:5,:], _emb.cpu().detach().numpy()[:5,:])
